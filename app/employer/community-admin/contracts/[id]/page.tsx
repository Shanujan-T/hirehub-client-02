  ...
  return (
    <CommunityAdminRoute>
      <DashboardPortalShell
        ...
        backHref="/employer/community-admin/contracts"
        ...
        {loading || !contract ? (
          ...
          <Card className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <Link href={hrefWithReturn(`/employer/community-admin/contracts/${contract.id}/messages`)}>
                <Button variant="outline" size="sm" className="rounded-full">Messages</Button>
              </Link>
              {contract.status === "open_internally" && (
                <Link href={hrefWithReturn(`/employer/community-admin/contracts/${contract.id}/applicants`)}>
                  <Button variant="gradient" size="sm" className="rounded-full">Assign Member</Button>
                </Link>
              )}
              {contract.status === "submitted" && (
                <Link href={hrefWithReturn(`/employer/community-admin/contracts/${contract.id}/review`)}>
