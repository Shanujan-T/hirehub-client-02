...
function ContractDetailContent() {
  ...
  return (
    <AuthenticatedRoute>
      <DashboardPortalShell title="Contract Details" backHref="/user/contracts" backLabel="Back to contracts">
        {loading || !contract ? <LoadingState /> : (
          <Card>
            ...
            <Link href={hrefWithReturn(`/user/contracts/${contract.id}/messages`)} className="mt-4 inline-block">
            ...
            {contract.status === "completed" && (
              <Link href={hrefWithReturn(`/user/reviews/${contract.id}`)} className="mt-4 inline-block">
