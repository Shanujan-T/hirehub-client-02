...
export function ProfileAccountVerificationSection(_props: ProfileAccountVerificationSectionProps = {}) {
  ...
  if (status === "pending") {
    return (
      <div id={PROFILE_ACCOUNT_SECTION_ID} className="scroll-mt-24">
        <Card className="space-y-3 border-border p-5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="text-base font-bold text-foreground">Account Verification</h2>
            <StatusBadge status="pending" kind="account" />
          </div>
          <p className="text-sm leading-relaxed text-muted">
            A previous legacy identity (document) submission is still awaiting platform review. Current account
            verification uses a phone or email code — contact support if this state persists.
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div id={PROFILE_ACCOUNT_SECTION_ID} className="scroll-mt-24">
      <Card className="space-y-5 border-border p-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-base font-bold text-foreground">Account Verification</h2>
          <StatusBadge status="unverified" kind="account" />
        </div>
        <p className="text-sm font-medium text-foreground">Not verified</p>
        <p className="text-sm leading-relaxed text-muted">
          Confirm your phone number or email to show this account is reachable before creating a community. One
          one-time code is enough — no government ID or documents are collected.
        </p>
...
/** @deprecated Use ProfileAccountVerificationSection */
export const ProfileIdentityVerificationSection = ProfileAccountVerificationSection;
